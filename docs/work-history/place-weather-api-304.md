...
#304 단계에서는 실제 API 호출 코드·응답 검증·재시도·Provider를 구현했고 화면은 변경하지 않았다. 이후 [#306](place-weather-ui-306.md)에서 기온·습도·아이콘과 상태·재호출 UI를 기존 헤더에 연결했다. #307에서는 사용자 요청으로 격자 누락 시 판교역 기본 위치를 조회한다. 실제 장소 좌표 연동은 계약 확정 후 진행한다. 기상청 데이터는 지역의 실외 기상 정보이며 실내·토양 측정값이 아니다.
