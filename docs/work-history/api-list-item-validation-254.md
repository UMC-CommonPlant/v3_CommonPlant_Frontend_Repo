...
clean 작업 트리와 원격 `develop`, 이슈 중복을 확인하고 기존 #254를 재사용했습니다. 작업 시작 시 #253은 Closed이고 이슈·PR #263의 Project 상태는 Done, 상위 #226은 17/20 완료였습니다. 이번 문서 변경에서 #253의 병합 전 표기를 정정했습니다.
...
- #248~#253 보호와 정상 빈 목록·확인된 wrapper 호환성, 미사용 공용 위젯 5개·public 버튼 variant를 유지합니다. 사용자 병합 후 최신 `develop`에서 #255 → #256 순서로 진행합니다.
...
| 이 문서의 최종 커밋 | #253 병합 상태 정정, #254 계약·검증·남은 제한과 현행 문서 갱신 | 로컬 Markdown 링크·문서 인덱스 확인, `git diff --check` |
