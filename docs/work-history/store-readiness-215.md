...
2026-09-23 후속 사용자 지시로 Apple 쪽 로그인·계정 확인은 우선 보류한다. 사용자가 재개하기 전에는 Apple 로그인을 반복 요청하거나 Android 준비의 선행 조건으로 두지 않는다. 기존 Apple 구현·Profile/Release 설정을 변경하거나 Apple 검증을 완료 처리하는 뜻은 아니다. Android의 계정 확인·서명·배포 준비를 우선한다.

Play Console의 기기 인증에는 Android 10 이상·루팅되지 않은 실물 Android 모바일 기기가 필요하다. 에뮬레이터와 iPhone은 이 인증을 대체하지 않는다. 개발자 계정을 만든 **소유자 계정**으로 웹과 Play Console 모바일 앱에 로그인해 웹의 기기 확인 QR 안내를 따라야 한다. 관리자 권한만으로 소유자를 대신하지 않는다. 등록 전화번호와 기기 전화번호는 같을 필요가 없으며 같은 기기를 계속 보유할 의무도 없다. 새 기기 구입을 전제로 하지 않는다. [Google 기기 인증 안내](https://support.google.com/googleplay/android-developer/answer/14316361?hl=en)

이 요구는 현재 계정의 스토어 배포 준비 중 기기 인증에 관한 것이다. 기기가 없어도 코드·문서·빌드 설정 점검은 진행할 수 있으며, 기기 인증과 실제 앱 QA는 각각 확인한다.
...
| Apple 계정 | 로그인 화면까지 확인. 이후 사용자 지시로 우선 보류 | 사용자가 재개하면 Developer Program 팀·역할·App ID·앱·빌드 이력 확인. Android 준비와 분리 |
...
| 순서 | 작업과 완료 기준 | 수정할 영역 |
| --- | --- | --- |
| 1 | Play 인증 완료 후 Android 앱/빌드 이력 우선 확인. Apple 팀/역할·앱/빌드 이력은 사용자 재개 후 확인 | 이 문서·#215, RELEASE-02-B/03 |
...
- Apple 계정 확인은 사용자 보류다. Android 준비는 계속 분리해 진행하며 Play 소유자 인증·서명·빌드 이력 미확인 항목은 완료 처리하지 않는다.
...
| `aead445` | README·문서 인덱스·배포 가이드·후속/화면 계획의 재개 상태 동기화, 문서 검증 결과 기록 |
