...
- 저장소·콘솔·공식 문서 조사와 Markdown 변경만 수행한다. Flutter 애플리케이션 동작, Release 빌드, 실제 업로드·로그인·사진 QA는 이번 점검에서 실행하지 않는다.
- 문서 검증은 `git diff --check`, 상대 파일/heading 링크 및 개인 iOS 파일 보존 확인으로 수행한다.
