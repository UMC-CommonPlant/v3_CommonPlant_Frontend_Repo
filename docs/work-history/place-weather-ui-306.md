...
> 현재 정책 #309: 장소 주소에 대응하는 좌표로만 조회하며, 좌표 없으면 조회 불가를 안내한다. #307의 판교역 기본값은 철회했다. 아래 준비 중 문구는 당시 기록이며 [현재 정책](place-weather-unavailable-309.md)을 따른다. [#307의 실제 API 검증](place-weather-fallback-307.md)과 예보 시각 수정은 유지한다.
...
| 3 화면 | 기존 헤더의 환경 수치를 날씨 영역으로 교체, 준비 중/로딩/성공/오류·같은 자리 재호출 버튼 | 작은 휴대폰·큰 글씨·접근성, 연속 탭·장소 변경·기존 화면 회귀 테스트 후 `Feat` 커밋 | 완료 |
| 4 마감 | 실제 커밋·검증·구현/미구현 경계를 중앙 문서와 PR에 반영 | format/analyze/test, CI, Project 10, `Docs` 커밋 | 완료 (최신 CI는 PR 체크 참조) |

각 구현 단계는 코드와 관련 테스트를 함께 커밋한다. 전체 검사는 마지막에 순차 실행하며 변경/실패가 생긴 범위만 필요한 만큼 다시 검사한다. PR 병합은 사용자가 한다.

## 화면과 상태 기준

- 기존 `PlaceDetailHeader`의 제목 오른쪽 환경 영역을 날씨 영역으로 교체한다. 제한된 폭에서 줄바꿈을 허용하고, 14pt 토큰이 20pt 이상으로 확대되는 큰 글씨에서는 제목 아래 전체 폭에 날씨를 배치해 가독성을 유지한다. 실패·로딩도 같은 영역을 사용한다. 친구 목록·장소 제목을 보존한다.
...
| `7fce7e6` | 표시 모델·장소 격자 경계·실패 재조회 | format, 표시/상태 테스트 16개 통과 |
| `a0c99b8` | 헤더 상태 UI·같은 위치 재호출·글씨 확대·fixture 수치 제거 | 새 widget test 11개, 전체 751개 통과·Linux golden 1개 로컬 스킵 |
...
마감 문서 커밋은 현재 화면/조회 상태를 중앙 문서에 동기화한다. 이슈 #306과 PR #305를 Project 10 Place로 관리하며, 병합은 사용자에게 남긴다. 원본 작업 디렉터리의 개인 iOS 설정은 변경하지 않았다.
