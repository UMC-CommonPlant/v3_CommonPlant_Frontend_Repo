...
| --- | --- | --- | --- |
| 1 문서 | 작업 방식, 상태별 동작, 백엔드 변경 표를 먼저 기록 | 문서 링크·diff 검사 후 `Docs` 커밋 | 진행 중 |
| 2 조회·표시 경계 | 장소 ID → 기상청 격자 공급 지점, 화면용 상태, 값/아이콘/시각 표시 모델, 재조회 중복 차단 | 로컬/좌표 없음은 호출 0회, loading/error/data 전달, 오래된 값 제외, 재조회 단위 테스트 후 `Feat` 커밋 | 예정 |
...
| 커밋 | 변경 | 검증 |
| --- | --- | --- |
| 첫 문서 커밋 | 사전 계획과 협업 기준 | `git diff --check` |
