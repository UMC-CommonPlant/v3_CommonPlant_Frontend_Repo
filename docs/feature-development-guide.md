...
Controller 결과도 현재 화면이 구분해야 하는 정보만 표현합니다.

- 성공과 실패만 구분하고 전달할 값이 없으면 `bool`을 사용합니다.
- 성공 뒤 목적지가 둘 이상이거나 식별자처럼 화면에 전달할 값이 있으면 result 타입을 사용합니다.
- 내부 상태 값을 이름만 바꿔 그대로 전달하는 getter나 wrapper는 추가하지 않습니다.
- 장래 확장 가능성만으로 enum, result, service, event 계층을 미리 만들지 않습니다. 실제 분기나 두 번째 구현이 생길 때 확장합니다.
...
감사에서 확인한 이미지 key 누락은 #248에서 Plant key 보존과 Place 사진 수정의 안전 차단으로, 계정별 캐시는 #249에서 세션 격리로 보완했습니다. #250은 입력 변경 중 제출 잠금을, #251은 원격 식물 등록 장소의 상태·선택·제출과 실제 조회 재시도를 보완합니다. API 비사용 fixture를 원격 조회의 loading/error/empty 대체값으로 사용하지 않습니다. #252는 장소 code 없는 원격 수정의 거짓 성공을 막고 성공 후 관련 조회를 갱신합니다. 현재 Plant 응답에 없는 code를 장소명으로 추측하지 않습니다. #253은 주소 결과의 출처·세션·화면 수명을 검증해 폼에 연결하며 API 모드 fixture를 차단했습니다. 실제 주소 검색은 별도 미결정 범위입니다. #254는 공용 목록 파서가 정상 빈 배열과 확인된 wrapper를 유지하면서, 비-Map 항목을 하나라도 포함한 응답은 항목 위치가 드러나는 오류로 처리합니다. 잘못된 항목을 제외해 빈 목록이나 부분 성공으로 바꾸지 않습니다. #255는 공용 입력의 실제 enabled 상태가 clear 표시·실행에도 적용되도록 보완해 비활성 값과 콜백을 보존합니다. #256은 수정 폼의 API mode 선택과 순수 변환을 폼 진입 Provider에 두고, fetch·retry·테스트 override는 각 원본 원격 Provider 하나에 둡니다. 자세한 기준은 [상태관리 가이드](state-management-guide.md#수정-폼의-원격-정보-경계)를 따릅니다. 위 기준을 이미 모든 경로가 충족한 것으로 해석하지 않습니다.
...
- MVP 앱명 `커먼플랜트`와 Android 식별자 `com.plant.common`, iOS Bundle ID `com.commonplant.app`을 사용합니다(#295 사용자 변경 요청 반영). 별도 설치·배포 채널이 필요해지면 dev/staging flavor를 `docs/release-workflow.md` 기준으로 검토합니다.
