...
1. `docs/figma-frame-map.md`에서 대상 route의 Figma frame 이름과 node-id를 먼저 확인합니다.
2. Figma 화면의 목적과 사용자 액션을 먼저 파악합니다.
3. 이미 있는 공용 위젯으로 대체 가능한 요소를 표시합니다.
4. 부족한 스타일 토큰이 반복 값인지 일회성 값인지 판단합니다.
5. 페이지 레이아웃을 `CommonScaffold` 또는 feature 전용 scaffold로 구성합니다.
6. 상태별 화면을 먼저 정의한 뒤 성공 상태 UI를 채웁니다.
...
2026-09-07 결정: 휴대폰(Android/iPhone) 최적화가 1순위입니다. iPad·Mac 설치를 막는 설정은 추가하지 않으며 iPad 전용 레이아웃과 최적화는 후순위로 둡니다. 큰 화면의 설치 가능성을 별도 최적화 완료로 표현하지 않습니다.
