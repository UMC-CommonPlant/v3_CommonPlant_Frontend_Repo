...
| Place | 친구 관리 | `/places/:placeId/friends` | `#2-3-2 친구 관리` | `1:3417` | 기본 | #32, #246 | #245 API 모드 실제 멤버 조회·검색, 변경 미지원 안내 |
| Place | 친구 관리 | `/places/:placeId/friends` | `#2-3-2 친구 관리 - 삭제 알럿 (리더` | `1:3491` | alert | #32 | fixture 모드에서 유지, API 모드는 삭제 endpoint 부재로 미노출 |
...
Figma 화면이나 route 계약이 확인된 정원과 마이만 실제 목적지로 연결합니다. 정보, 이야기, 캘린더는 탭을 비활성화하거나 임시 route를 만들지 않고, 선택하면 현재 화면에서 `준비 중인 기능입니다` Snackbar를 표시합니다.

| 탭 | 화면·Route | 현재 동작 | 추후 연결 조건 |
| --- | --- | --- | --- |
| 정보 | 없음 | 구현 예정 Snackbar | Information 화면·route 요구사항 확정 |
| 이야기 | 없음 | 구현 예정 Snackbar | Story 화면·route 요구사항 확정 |
| 정원 | Home `/` | 실제 화면 이동 | 구현 완료 |
| 캘린더 | 없음 | 구현 예정 Snackbar | Calendar 화면·route 요구사항 확정 |
| 마이 | User `/me` | 실제 화면 이동 | 구현 완료 |
