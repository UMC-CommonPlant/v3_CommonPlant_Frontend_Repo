...
| 프로필 이미지 선택 | User profile edit | 앨범 선택·미리보기·교체·로컬 선택 취소 연결 (#295) |
...
API 비사용 화면 확인 모드는 원격 인증 gate를 적용하지 않는다. `시작하기` 이후
`/login`에서 소셜 항목을 선택하고 기존 로컬 프로필·약관·홈 흐름을 확인할 수 있다.
이 모드의 소셜 선택은 실제 SDK·백엔드 인증을 수행하지 않는다. 온보딩 완료 값은
계속 보존하며, API 모드의 토큰 복원과 보호 route 정책은 기존대로 유지한다.
