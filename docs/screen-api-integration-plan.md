...
2026-09-19 [#302](https://github.com/UMC-CommonPlant/v3_CommonPlant_Frontend_Repo/issues/302)에서 문서화했고, 2026-09-20 [#304](https://github.com/UMC-CommonPlant/v3_CommonPlant_Frontend_Repo/issues/304)에서 공공데이터 조회 계층을 준비합니다.
...
- #304의 API·mapper·repository·Provider는 명시적인 기상청 격자를 받아 호출할 준비가 됐습니다. 실제 Place 화면은 아직 이 Provider를 구독하지 않으며 화면의 fixture 수치를 실제 날씨로 취급하지 않습니다.
...
후속 화면 연결에서는 실패한 **동일 날씨 위치에 재호출 아이콘**을 표시하고 loading 중 중복 탭을 막습니다. 좌표 계약과 함께 조회 시점·갱신/캐시·좌표 누락 UI를 정하고 휴대폰 레이아웃·접근성을 검증합니다. 현재 재호출 버튼과 기온·습도·아이콘 UI는 구현하지 않았습니다.
