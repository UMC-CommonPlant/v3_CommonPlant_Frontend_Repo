...
분기합니다. Apple 버튼은 #293부터 iPhone 앱에서만 표시하며 실제 Apple 서버 검증은 backend #152 배포
...
- 무인 CI 원격 E2E의 테스트 인증·데이터 격리·cleanup과 GitHub Environment. 개인 테스트 계정의 수동 소셜 로그인 검증은 #293에서 재개했으며 기기·설정 준비 여부를 별도로 기록
...
| 순서 | 수직 슬라이스 | 범위 | 상태 |
| --- | --- | --- | --- |
| P0 | Auth 로그인·회원가입 | 로그인 화면, 인증 세션, 프로필 등록, route redirect, `/auth/login`, `/auth/register` | #227 / PR #228 병합, 실제 SDK·`isNewUser` 계약은 #285 / PR #286 병합 완료. #293 자동 검증 보완·실계정 검증 대기 |
| P1 | Home 초기 데이터 | 인증 사용자 정보와 장소·식물 요약을 화면 상태로 연결 | #232 / PR #233 병합 완료 |
| P2 | Plant 핵심 동선 | 목록, 상세, 생성, 수정 API와 각 화면 상태 연결 | #229, #231 병합 완료 |
| P3 | User 프로필 | 내 정보 조회·수정과 프로필 화면 연결 | #237 / PR #238 병합 완료, #295 이미지 파일 선택·교체 후속 연결 |
...
| User | 마이페이지 `/me`, 설정 `/me/settings`, 회원 정보 수정 `/me/edit` | 조회·이름 수정·탈퇴와 세 화면 연결, #249 세션 격리·#250 제출 잠금 병합 | 실기기 이미지 QA·알림 영속화 | `GET/PUT/DELETE /users` 연결, Image·알림 API/정책 필요 | 연결·세션 격리·제출 잠금 PR 병합 |
...
| Place | 장소 등록 | create API·생성 code·친구 추가 route 연결, #250 잠금 병합, #253 / PR #263 주소 결과·검증·요청 연결 | 실제 주소 검색·이미지 실기기 QA | 결과 계약은 테스트로 검증, API 모드 fixture 주소 차단 | 실제 검색 미연결로 새 주소가 필요한 생성 동선 미완료 |
...
- Apple 버튼과 SDK 호출은 #293부터 iPhone 앱에서만 제공합니다. Android에서는 버튼·간격·Semantics를 렌더링하지 않으며 backend #152 배포 전까지 실제 Apple 인증은 Blocked입니다.
...
- #295는 앨범 사진 선택·미리보기·교체·초안 취소와 기존 optional multipart 제출을 연결합니다. 서버 사진의 명시적 삭제 UI는 포함하지 않습니다.
...
- 날씨 조회의 향후 책임은 #302에서 프런트엔드의 공공데이터 API 호출로 결정했습니다. 아래 정책은 후속 구현 기준이며 현재 날씨 표시가 연결됐다는 의미는 아닙니다.
...
2026-09-19 사용자 요청을 [#302](https://github.com/UMC-CommonPlant/v3_CommonPlant_Frontend_Repo/issues/302)에 기록합니다. 이번 작업은 문서화이며 API·DTO·Provider·UI 코드는 변경하지 않습니다.

확정한 방향:

- 장소 정보를 조회할 때 백엔드가 x/y 좌표를 함께 전달할 예정입니다. 최종 필드명이 정해지기 전까지 문서에서는 `xPosition`, `yPosition`으로 임시 표기합니다.
- 임시 명칭을 배포된 JSON 계약으로 사용하지 않습니다. 백엔드가 최종 명칭과 응답 계약을 확정하면 문서를 수정하고 DTO·mapper·날씨 조회 연결을 진행합니다.
- 날씨 정보는 프런트엔드가 공공데이터 API를 호출해 구현합니다. 백엔드는 장소 좌표를 제공하며, 날씨 결과 자체를 장소 응답에 추가하는 것을 선행 조건으로 두지 않습니다.

오류·지연 대응은 다음 **사용자 검토안**으로 기록합니다. 최대 3회는 한 번의 조회 흐름에서 최초 호출 1회와 재시도 최대 2회를 합친 총 시도 횟수입니다.

| 상황 | 검토 중인 동작 |
| --- | --- |
| 각 호출 시작 | 날씨 요청과 5초 제한 타이머를 race시킵니다. 사용자가 제안한 `setTimeout` + race의 의도는 시도마다 대기 시간을 최대 5초로 제한하는 것입니다. Flutter/Dart 구현 방식은 후속 구현에서 정합니다. |
| 5초 전에 성공 | 날씨 정보를 표시하고 추가 시도를 중단합니다. |
| 호출 오류 또는 5초 초과 | 실패한 시도로 처리하고 총 3회 한도 안에서 다음 호출을 시도합니다. |
| 총 3회 모두 실패 | 자동 시도를 종료하고 **날씨 정보가 표시될 동일한 위치에 재호출 아이콘 버튼**을 노출합니다. 별도 위치의 버튼으로 옮기지 않습니다. |
| 재호출 아이콘 탭 | 날씨 조회를 다시 시작하는 수동 재시도 진입점으로 사용합니다. |

후속 구현 전에 확인할 항목:

- 좌표가 포함될 장소 조회 endpoint와 응답 위치, 최종 필드명, 숫자 타입·nullable 여부, x/y의 의미·좌표계·단위. 위도/경도 또는 공공데이터 격자 좌표라고 추정하지 않습니다.
- 사용할 공공데이터 API·요청 및 응답 계약·인증키 주입 방식과 필요한 좌표 변환. 현재 값이나 endpoint를 임의로 선정하지 않습니다.
- 표시할 날씨 항목, 조회 시점·갱신 주기·캐시, 좌표 누락 및 정상 빈 응답 처리, 재시도 사이의 간격, 수동 재호출의 시도 횟수 초기화 규칙. 이번 문서에서는 추가 정책을 확정하지 않습니다.

구현 시에는 기존 feature 계층과 상태관리를 활용하고 API 파싱·재시도 처리를 화면 위젯에 넣지 않습니다. 날씨 영역의 loading·success·실패 후 재호출 상태를 구분하며, 늦은 응답·장소 또는 계정 전환·중복 탭이 이전 결과를 표시하거나 3회 한도를 넘는 중복 호출로 이어지지 않는지 검증합니다. 다른 계층의 자동 재시도와 중첩되지 않는지도 확인합니다.

추후 검증 대상은 시도별 5초 제한, 1·2·3번째 성공 시 중단, 오류·timeout 혼합 후 총 3회 실패, 동일 위치 재호출 아이콘 및 수동 재호출입니다. 현재 코드에는 좌표 파싱과 실제 날씨 요청이 없으며, 기존 fixture 환경 수치를 실제 날씨로 취급하지 않습니다. 백엔드 확인 항목은 [PLACE-07](backend-api-open-questions.md#place-07-장소-조회-좌표-계약)에서 추적합니다.
...
- 실제 주소 검색 adapter는 미연결이며 이미지 선택은 #295에서 도메인 multipart에 연결했습니다. 주소 선택 결과 전달은 #253에서 구현·검증했지만 API 모드의 실제 검색은 미연결입니다. 샘플 결과를 원격 폼에 반영하지 않고 기존 주소를 보존합니다([검증·제한](work-history/place-address-result-253.md)). #248의 사진 보존 가드는 유지하며 #295에서 새 사진 파일을 명시적으로 선택한 교체만 허용합니다. 파일 미선택 정보 수정은 key 조회 계약 확보가 필요합니다.
...
가입·회원 정보·식물·장소의 파일 선택/미리보기/교체는 [#295 작업 기록](work-history/form-image-selection-295.md)을 기준으로 합니다. 자동 테스트와 native 실기기·서버 저장 QA는 구분하며 메모 이미지 보류는 유지합니다.
