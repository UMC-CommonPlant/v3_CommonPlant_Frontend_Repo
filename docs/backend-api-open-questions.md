...
- 프론트 반영: #304에서 공공데이터포털 초단기실황(현재 기온·습도·강수형태) + 초단기예보(하늘상태) 조회 계층을 준비했다. 최종 계약이 확정되면 임시 명칭을 수정하고 Place DTO·좌표 변환·화면을 연결한다. 확정 전에는 임시 JSON 필드 파싱이나 좌표 변환을 추가하지 않는다. 시도별 5초·총 3회는 조회 계층에 구현했으며 동일 날씨 위치 재호출 UI는 후속이다. [현재 조회 방침](screen-api-integration-plan.md#장소-좌표프런트-날씨-조회-방침-302)을 따른다.
