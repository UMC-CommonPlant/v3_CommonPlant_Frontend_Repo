...
- 프론트 반영: #304에서 공공데이터포털 초단기실황(현재 기온·습도·강수형태) + 초단기예보(하늘상태) 조회 계층을 준비했다. 최종 계약이 확정되면 임시 명칭을 수정하고 Place DTO·좌표 변환을 이미 구현된 날씨 화면에 연결한다. 확정 전에는 임시 JSON 필드 파싱이나 좌표 변환을 추가하지 않는다. 시도별 5초·총 3회는 조회 계층에 구현했으며 #306에서 동일 날씨 위치 재호출 UI와 중복 탭 차단까지 구현했다. #309 사용자 결정으로 장소 주소의 좌표가 없으면 조회 불가를 안내하며 요청하지 않는다. 판교역 기본값은 제거했다. 주소 문자열 자체로 요청하거나 새 지오코딩 서비스를 추측해 연결하지 않는다. 확정 후 [변경 파일·검증 표](work-history/place-weather-ui-306.md#백엔드-확정-후-수정할-영역)를 따라 후속 구현한다. [현재 조회 방침](screen-api-integration-plan.md#장소-좌표프런트-날씨-조회-방침-302)을 따른다.
