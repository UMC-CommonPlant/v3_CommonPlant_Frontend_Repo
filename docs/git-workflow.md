...
2026-09-20부터 구현 작업은 **문서 기록 → 구현 세분화 → 커밋 단위 구현·검증** 순서로 진행합니다. 먼저 작업 이력 문서에 단계별 범위·완료 조건·검증 방법을 작성하고 문서 커밋을 남깁니다. 이후 각 단계의 코드와 테스트를 함께 커밋합니다. 백엔드 미확정 항목은 확정 후 수정할 파일·계약·검증을 표로 남기며, 독립적으로 가능한 UI/상태 구현과 실제 서버 연결을 분리합니다. 문서·코드·서버 검증 완료는 각각 기록합니다.

여러 상태 경계나 계층을 함께 수정하는 Task는 구현 전에 이슈 본문 또는 실행 계획 문서에 커밋 계획을 먼저 작성합니다. 하나의 Task를 무조건 한 커밋으로 합치지 않고, 리뷰어가 각 단계의 의도와 검증 결과를 독립적으로 확인할 수 있는 단위로 나눕니다.

권장 분리 순서:

1. 작업 기준이나 실행 계획 문서 갱신
2. State/model public API와 해당 unit test
3. Controller/repository 로직과 해당 unit test
4. page/widget 연결과 widget test
5. 전체 검증 결과와 커밋별 작업 이력 문서화

모든 Task가 위 다섯 커밋을 기계적으로 가져야 하는 것은 아닙니다. 파일 수가 적거나 한 변경 없이는 다른 단계가 컴파일되지 않는 경우에는 합칠 수 있지만, 아래 기준은 지킵니다.

- 각 구현 커밋은 가능한 범위에서 format과 관련 대상 테스트를 통과한 상태로 남깁니다.
- 테스트는 검증 대상 구현과 같은 커밋에 포함하거나, 테스트만으로 독립적인 의도가 있을 때 별도 `Test` 커밋으로 둡니다.
- 단순 import 이동이나 이름 변경은 해당 원인을 만든 구현 커밋에 포함합니다.
- 계획 문서의 작업 이력에는 커밋 해시, 변경 범위, 검증 명령을 한 행씩 기록합니다.
- 이슈 완료 코멘트에도 같은 커밋 목록과 전체 검증 결과를 남깁니다.
- 작업 이력만 갱신하는 마지막 문서 커밋은 자기 자신의 해시를 같은 커밋에 기록하지 않아도 됩니다.
