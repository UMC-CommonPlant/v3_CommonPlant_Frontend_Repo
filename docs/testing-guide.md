...
- TEST-02-A는 #203에서 API 비사용 Home → 장소 친구 요청 Android smoke와 수동 workflow로 도입했고, #289에서 로컬 완료 값을 초기화한 뒤 온보딩 → 소셜 선택 → 프로필·약관 → Home 구간을 앞에 추가했습니다. #218에서 `develop` run 3회의 연속 성공과 로그 구분을 확인했고, #224 결정에 따라 Android smoke는 수동으로 유지합니다.
