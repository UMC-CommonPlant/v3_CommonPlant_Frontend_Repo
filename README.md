...
2026-08-31 `develop`의 PR #272 병합 상태(`b2dc912`)를 기준으로 합니다. Epic #226의 하위 이슈 20/20과 감사 #248~#256이 완료되어 Epic과 Project 상태를 `Done`으로 종료했습니다. 이는 실제 인증 E2E나 모든 화면 동선의 완성을 뜻하지 않으며, 후속 실행 범위는 [화면·모델·API 연결 계획](docs/screen-api-integration-plan.md#후속-개발-실행-순서-267)에서 관리합니다.
...
3. #273에서 기존 Place API의 정확한 plant ID 대조로 Plant 소속 장소 code를 복원했습니다. 식물 검색 API는 백엔드 #92 완료 전까지 API 모드 fixture를 차단하고 미연결 안내를 유지합니다.
