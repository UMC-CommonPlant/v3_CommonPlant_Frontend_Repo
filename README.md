...
2026-08-31 `develop`의 PR #274 병합 상태(`58a741b`)를 기준으로 합니다. Epic #226의 하위 이슈 20/20과 감사 #248~#256이 완료되어 Epic과 Project 상태를 `Done`으로 종료했습니다. 이는 실제 인증 E2E나 모든 화면 동선의 완성을 뜻하지 않으며, 후속 실행 범위는 [화면·모델·API 연결 계획](docs/screen-api-integration-plan.md#후속-개발-실행-순서-267)에서 관리합니다.
...
4. #275에서 표준 API 오류와 필드 오류를 안전하게 매핑하고, 확인된 access token 오류 `A003`·`A004`·`A009`는 현재 세션을 종료해 로그인 안내로 연결합니다. refresh·서버 로그아웃은 백엔드 #149 계약 전까지 재시도하거나 추정하지 않습니다.
