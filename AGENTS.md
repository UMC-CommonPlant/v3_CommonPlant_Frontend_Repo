...
- GitHub 이슈, PR, 코멘트, Project 생성 및 수정은 인증된 `gh` CLI를 기본으로 사용한다. `gh`가 지원하지 않거나 사용자가 명시적으로 요청한 경우에만 GitHub 앱 등 대체 도구를 사용한다.
...
- 구현 작업은 **문서 기록 → 구현 세분화 → 코드·테스트를 커밋 단위로 진행**한다. 먼저 작업 이력 문서에 단계별 범위·완료 조건·검증을 기록하고 문서 커밋을 남긴다.
- 백엔드 미확정 사항은 지금 가능한 작업과 분리한다. 확정 후 바꿀 파일/모델/Provider, 필요한 계약, 재검증 항목을 문서 표로 남기며 독립적으로 구현 가능한 UI까지 일괄 보류하지 않는다.
...
- 신규 일반 이슈와 PR은 `ywkim95`, `bbielo`를 assignees로 지정한다.
