...
2. 기존 `lib/shared/widgets`와 `lib/core/theme` 토큰으로 해결 가능한지 확인한다.
3. 라우팅, 상태관리, API 모델, 테스트 영향 범위를 먼저 판단한다.
4. mock 데이터와 실제 API 연동 코드를 섞지 않는다.
5. loading, success, empty, error 상태를 명확히 나눈다.
